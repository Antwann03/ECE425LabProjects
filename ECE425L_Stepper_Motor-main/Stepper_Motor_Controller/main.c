	...
	while(1)
	{
			if(enable == 0)
		{
		 ...
		 else if (enable == 1)
			{
				...
				GPIOB ->DATA = (GPIOB -> DATA &~ 0x0F) | half_step[step_index]; //Sets GPIOB output to current step
				step_index = step_index - 1; //Moves to the next step
				SysTick_Delay1us(1200); //Delay for steps
			}
...
switch (pmod_btn_status)
	{
		...
		case 0x04:
		{
			//Enables the stepper motor to start rotating
			 enable = 1; 
			break;
		}
		...
		case 0x08:
		{
			//Disables the stepper motor from rotating
		 enable = 0;
			break;
		}
		...
		case 0x10:
		{
			//Toggles the direction of the stepper motor's rotation between CCW and CW
			direction_select ^= 1;
			break;
		}
