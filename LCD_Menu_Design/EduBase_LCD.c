...
void EduBase_LCD_Ports_Init(void)
{
	...
	//Configure teh PA5, PA4, PA3, and PA2 pins as output
	//by setting Bitd 5 to 2 in the DIR register
	GPIOA -> DIR |= 0x3C;
	...
	//by cleaing Bit 6 in the DATA register
	GPIOC ->DATA &= ~(0x40);
	
	//Enable the clock ot Port E by setting the
...
void EduBase_LCD_Pulse_Enable(void)
{
	...
	//during a read / write opeation (prage 49 of HD44780 datasheet)
	GPIOC -> DATA |= 0x40;
	SysTick_Delay1us(1);
	GPIOC -> DATA &= ~(0x40);
}
...
void EduBase_LCD_Write_4_Bits(uint8_t data, uint8_t control_flag)
{
	...
	//Ckear the LCD data lines (PA2 - PA5) and provide a 1 ms delay
	GPIOA -> DATA &= ~(0x3C);
