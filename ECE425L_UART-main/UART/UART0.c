...
void UART0_Init(void)
{
	...
	//	Enable the clock ot Port A by setting the
	...
	//	Select one stop bit to be transmitted at the end of a UART fram by clearing the STP2
	...
	//	Configure the PA1 pins to operate as a U0TX pin by writting 0x1 to the PMC1 field 
	//	(Bits 7 to 4) in the PCTL register.
	GPIOA -> PCTL |= 0x00000010;
	
	//	Configure the PA0 to operate as a U0RX pin by writting 0x1 to the PMC0 field 
	//	(Bits 3 to 0) in the PCTL register.
	GPIOA -> PCTL |= 0x00000001;
	
	//	Enable the digital functionality for PA1 and PA0 pins by setting Bits 1 to 0
	//	in the DEN register.
	GPIOA -> DEN  |= 0x03;
}
...
{
	//	User recursion to convert decimal number
