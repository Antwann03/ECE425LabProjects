...
//Declare the user-define function prototype for EduBase_Button_Interrupt
void EduBase_Button_Handler(uint8_t edubase_button_status);

//Initialize a global variable for an 8-bit counter
static uint8_t counter = 0;

//Declare  the function prototype for the fcuntion that calculates
//stopwatch value and stores it in an array
void Calculate_Stopwatch_Value(uint8_t stopwatch_value[]);

//Decalre the function prototype for the user-defined function for Timer 0A
...
int main(void)
{
	...
	//Initialize the Seven Segment Display (Port B and Port C)
	Seven_Segment_Display_Init();
	
	//Initialzie the SW2 and SW3 on the EduBase board with interrupts enabled (Port D)
