...
{	
	//Return form the function if the specified duty_cycle is greater than 
	...
	// Clear the PMC2 fiedl (Bits 11 to 8) in the RCTL register
	GPIOF -> PCTL &= ~0x00000F00;
	
	//Configure the PF2 pin to operate as a Module 1 PWM6 pin (M1PWM6)
	//by writing 0x5 to the PMC2 fiedl (Bits 11 to 8) in the PCTL register
	//The 0x5 value is deirved from Table  23-5 in the TM4C123G Microcontroller Datasheet
	GPIOF -> PCTL |= 0x00000500;
	
	//Enable the digital fucntionality for the PF2 pin
	...
	//Set the ACTMPAD field (Bits 7 to 6) to 0x3 in the PWM3GENA register
	...
	//Set the period by writing to the LOAD fiedl (Bits 15 to 0)
	...
	//Set the duty cycle by writting to the COMPA field (Bits 15 to 0)
	//in the PWM3CMPA register. When the counter matches the value in the register,
	//the PWM signal will be driven high
	PWM1 -> _3_CMPA = (duty_cycle - 1);
	
	//Enable the PWM1_3 block after confiugration by setting the 
...
{
	//Set the duty cycle by writting to the COMPA field (Bits 15 to 0)
	//in the PWM3CMPA regiter. When the counter matches the value in this register,
	//the PWM signal will be driven high
	PWM1->_3_CMPA = (duty_cycle - 1);
}
