...
{	
	...
	//Configure the PF2 pin to use the alternate function (M0 PWM0)
	//by setting Bit 6 in the AFSEL register
	GPIOB -> AFSEL |= 0x40;
	
	// Clear the PMC6 field (Bits 27 to 24) in the RCTL register
	...
	//The 0x4 value is deirved from Table  23-5 in the TM4C123G Microcontroller Datasheet
	GPIOB -> PCTL |= 0x04000000;
	
	//Enable the digital fucntionality for the PB6 pin
	...
	//Set the ACTMPAD field (Bits 7 to 6) to 0x3 in the PWM0GENA register
	...
	//in the PWM0CMPA register. When the counter matches the value in the register,
	//the PWM signal will be driven high
	PWM0 -> _0_CMPA = (duty_cycle - 1);
	
	//Enable the PWM0_0 block after confiugration by setting the 
...
{
	//Set the duty cycle by writting to the COMPA field (Bits 15 to 0)
	//in the PWM0CMPA regiter. When the counter matches the value in this register,
	//the PWM signal will be driven high
	PWM0->_0_CMPA = (duty_cycle - 1);
}
